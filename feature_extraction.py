...
def extract_features_from_epochs(epochs, fs, epoch_length):
    ...
    N = epochs.shape[0]
    L = epoch_length * fs # signal length 100Hz = sampling freq (100 amplitude values recorded every 1 sec) , epoch_length = 30(Each hypnogram value recorded after 30 sec) Hence 3000 values of amplitude correspopnd to one label.
  ...
  #  print(beta_indices)
    ...
    features = np.zeros((N, nr_features))    # calculation of delta, theta, alpha and beta band power ratios
    for index in range(N):    
        epoch = epochs[index, :]
        Y = abs(np.fft.fft(epoch))
        mean_total_power = np.mean(Y[all_indices])
        features[index,:] = (mean_total_power,
                             np.mean(f[all_indices] * Y[all_indices]) / mean_total_power, 
                             np.mean(Y[delta_indices]) / mean_total_power,
                             np.mean(Y[theta_indices]) / mean_total_power, 
                             np.mean(Y[alpha_indices]) / mean_total_power,
                             np.mean(Y[beta_indices]) / mean_total_power)
